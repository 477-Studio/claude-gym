...
func runStudio() {
	fmt.Println("Claude Gym Studio")
	fmt.Println("Asset development environment with hot reload")

	rl.SetConfigFlags(rl.FlagWindowResizable)
	rl.InitWindow(screenWidth*windowScale, screenHeight*windowScale, "Claude Gym Studio")
	...
	// Picker state: 0=none, 1=animation
	pickerMode := 0
	pickerIndex := 0

	// Start with Coffee, enable loop mode for studio
	animations.SetAnimation(AnimCoffee)
	...
	for !rl.WindowShouldClose() {
		...
		// Draw scene (no game UI)
		renderer.Draw(animations.GetState(), nil)

		// Draw studio UI
		drawStudioUI(animations, renderer, paused, speed, showHelp, pickerMode, pickerIndex)
...
func drawStudioUI(anim *AnimationSystem, r *Renderer, paused bool, speed float32, showHelp bool, pickerMode, pickerIndex int) {
	state := anim.GetState()
	...
	// Right: Biome label
	rl.DrawText("Office", screenWidth-int32(rl.MeasureText("Office", 8))-4, 3, 8, bright)

	// === PICKER (if open) ===
	if pickerMode > 0 {
		allAnims := anim.GetAllAnimations()
		items := make([]string, len(allAnims))
		for i, a := range allAnims {
			items[i] = a.String()
		}
		title := "Animation"
